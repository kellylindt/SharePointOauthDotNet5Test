...
namespace ConsoleApp1
{
    ...
    public class SharepointAuthenticationManager : IDisposable
    {
        ...
        public ClientContext GetContext(Uri web, string userPrincipalName, SecureString userPassword, string azureApplicationId)
        {
            var context = new ClientContext(web);

            context.ExecutingWebRequest += (sender, e) =>
            {
                string accessToken = EnsureAccessTokenAsync(new Uri($"{web.Scheme}://{web.DnsSafeHost}"), userPrincipalName, new System.Net.NetworkCredential(string.Empty, userPassword).Password, azureApplicationId).GetAwaiter().GetResult();
        ...
        public async Task<string> EnsureAccessTokenAsync(Uri resourceUri, string userPrincipalName, string userPassword, string azureApplicationId)
        {
            ...
            if (accessTokenFromCache == null)
            {
                ...
                try
                {
                    ...
                    string accessToken = await AcquireTokenAsync(resourceUri, userPrincipalName, userPassword, azureApplicationId).ConfigureAwait(false);
        ...
        private async Task<string> AcquireTokenAsync(Uri resourceUri, string username, string password, string azureApplicationId)
        {
            string resource = $"{resourceUri.Scheme}://{resourceUri.DnsSafeHost}";

            var clientId = azureApplicationId;
